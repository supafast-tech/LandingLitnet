...
import { getPagesWord } from '../../utils/pluralUtils';
import characterImage from "figma:asset/aabdbf108ebf8325b23f5ced29ab2d44309811a5.png";
import { useIsMobile } from '../../hooks/useIsMobile';

interface StatsSlide5Props {
  stats: StatsData | null;
  urlParams: Record<string, string>;
}

export function StatsSlide5({ stats, urlParams }: StatsSlide5Props) {
  const isMobile = useIsMobile();
  const paramName = stats?.slide5_pages_param || 'pages_count';
  const pagesCount = parseInt(urlParams[paramName] || '12500');
  const pagesWord = getPagesWord(pagesCount);
  
  const mainText = stats?.slide5_text || `Вы прочитали {число} в 2025 году.`;
  const cardText = stats?.slide5_card_text || 'Если бы это была одна книга, она бы точно попала в список самых длинных романов!';
    ...
    <div className="relative w-full h-screen overflow-hidden flex items-center justify-center">
      ...
      <div className="relative z-10 flex items-center w-full h-full px-4 md:px-20">
        ...
        <div 
          ...
          style={{ 
            zIndex: 5, 
            bottom: isMobile ? 'calc(32pt - 80pt + 32pt - 40pt + 40pt)' : '-136pt',
            left: isMobile ? '0' : 'auto',
            right: isMobile ? '0' : '-64pt',
            transform: isMobile ? 'scale(1.4)' : 'none',
        ...
        >
          ...
          >
            {processText(mainText, pagesCount, pagesWord)}
          </h2>
