...
import characterImage from "figma:asset/e16a05ddb857211c6ebc89dac2d7cfd82130132a.png";
import { useIsMobile } from '../../hooks/useIsMobile';

interface StatsSlide4Props {
  stats: StatsData | null;
  urlParams: Record<string, string>;
}

export function StatsSlide4({ stats, urlParams }: StatsSlide4Props) {
  const isMobile = useIsMobile();
  const paramName = stats?.slide4_library_param || 'library_count';
  const libraryCount = parseInt(urlParams[paramName] || '250');
  const bookWord = getBookWord(libraryCount);
  
  const mainText = stats?.slide4_text || `В вашей библиотеке {число}.`;
  const cardText = stats?.slide4_card_text || 'Это целая вселенная историй, готовых к прочтению в любой момент. Что выберете следующим?';
    ...
    <div className="relative w-full h-screen overflow-hidden flex items-center justify-center">
      ...
      <div className="relative z-10 flex items-center w-full h-full px-4 md:px-20">
        ...
        <div 
          ...
          style={{ 
            zIndex: 5, 
            bottom: isMobile ? 'calc(192pt - 80pt + 32pt)' : '-128pt',
            left: isMobile ? '-128pt' : 'auto',
            right: isMobile ? 'auto' : '256pt',
            transform: isMobile ? 'scale(2.3)' : 'scale(1.8)',
        ...
        >
          ...
          >
            {processText(mainText, libraryCount, bookWord)}
          </h2>
