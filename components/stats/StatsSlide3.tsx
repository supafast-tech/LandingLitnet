...
import { getSubscriptionWord } from '../../utils/pluralUtils';
import characterImage from "figma:asset/730eb2106796a19ad59e5e5bc3bc6b2058d0fd6e.png";
import { useIsMobile } from '../../hooks/useIsMobile';

interface StatsSlide3Props {
  stats: StatsData | null;
  urlParams: Record<string, string>;
}

export function StatsSlide3({ stats, urlParams }: StatsSlide3Props) {
  const isMobile = useIsMobile();
  const paramName = stats?.slide3_subscriptions_param || 'subscriptions_count';
  const subscriptionsCount = parseInt(urlParams[paramName] || '35');
  const subscriptionWord = getSubscriptionWord(subscriptionsCount);
  
  const mainText = stats?.slide3_text || `Вы подписались на {число} в 2025 году.`;
  const cardText = stats?.slide3_card_text || 'Возможно, вы нашли новые любимые голоса или следите за творчеством знакомых авторов. В любом случае, это отличная компаия!';
    ...
    <div className="relative w-full h-screen overflow-hidden flex items-center justify-center">
      ...
      <div className="relative z-10 flex items-center w-full h-full px-4 md:px-20">
        ...
        <div 
          ...
          style={{ 
            zIndex: 5, 
            bottom: isMobile ? 'calc(-56pt - 80pt + 32pt)' : '-200pt', 
            left: isMobile ? '0' : 'auto',
            right: isMobile ? '0' : '64pt',
            transform: isMobile ? 'scale(1.3)' : 'scale(1.35)',
        ...
        >
          ...
          >
            {processText(mainText, subscriptionsCount, subscriptionWord)}
          </h2>
